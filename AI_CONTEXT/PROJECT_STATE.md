...
## Delta (2026-08-22 #2, Claude Code)
**Skill `email-writer` + tri thức owner-unit IT (từ email dự án PFMP).** Phiên xuyên repo chạm spoke **PFMP** (Số hóa QLCT Xây lắp): ở spoke đối chiếu Chủ trương TGĐ (BPM+CRM) đã duyệt → mockup v18 + email nháp (xem `CROSS_REPO_LOG.md` + AI_CONTEXT spoke). Ở **hub**: [TT] đưa bản email tự sửa `Email fix.docx` → rút bài học, tạo skill **`03_Skills/email-writer/`** (văn phong + template + checklist, trỏ REF) và làm giàu **`REF-TPBANK-DELIVERY §1b`** (owner-unit IT: PM.IT·Dev1/3=BPM·Dev2=CRM·CBS=FCC·ITA.IT=kiến trúc·CA.CB=nghiệp vụ·DP/Data·CCP — cấp đơn vị, không handle cá nhân). INDEX +email-writer. **Blocker:** không. Danh mục skill: +email-writer.

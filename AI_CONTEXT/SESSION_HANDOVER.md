...
## Delta phiên (2026-08-20 — sheet cấu hình người nhận báo cáo email + debug teamlead, Claude Code)
- **Task completed:** Bỏ **hardcode người nhận** trong `ReportEmailService.gs` (SHTD) → chuyển sang **sheet cấu hình `Report_Config`** (đọc động, đổi người nhận không cần sửa code/redeploy). Thêm `setupReportConfig()` (chạy 1 lần từ Apps Script editor, no-overwrite), `_reportConfig_()` (đọc sheet, **fallback hằng số cũ** nếu sheet chưa tạo). Nghiệm thu **end-to-end LIVE**: `send_email.js --dry` xác nhận đọc từ `Report_Config (sheet)`, `Enabled=true`, To=`CuongVM1@tpb.com.vn`, Cc thật. **Debug thiếu teamlead** (đọc live `User_Master` 41 user qua action `user-list`): xác định 3 người [TT] mong Cc bị loại đúng logic — **MaiTTT7 + QuynhNNY** Teamlead active nhưng **Email trống**; **TuanTT4** Role=**Admin** (không phải Teamlead). Làm rõ `SPTDDN.CB@tpb.com.vn` là **Cc_Extra** chủ đích (không phải user).
- **Files changed:** *SHTD (commit `d38de2f`, main, đã push):* sửa `backend/ReportEmailService.gs` (+`setupReportConfig`/`_reportConfig_`, Cc nhiều role + Cc_Extra + gate Enabled), `verify_send_report.mjs` (+SR8/9/10). *AIOS (commit `8470f86`, master, đã push):* sửa `03_Skills/weekly-report/send_email.js` (`--dry` in nguồn cấu hình + Enabled).
- **Decision made:** (1) Cấu hình người nhận = **sheet `Report_Config`** (5 key: Enabled/To_Username/Cc_Role/Cc_Extra/From_Name), **fallback hằng số** khi sheet vắng → không vỡ bản đã deploy. (2) `Cc_Role` hỗ trợ **nhiều role** (phẩy); `Cc_Extra` cho hòm thư ngoài User_Master; `Enabled=false` chặn gửi thật (vẫn cho dry-run) — công tắc an toàn. (3) **TuanTT4 giữ Admin** (cần cho send-report + quản trị user) → thêm vào Cc qua **Cc_Extra**, KHÔNG đổi role sang Teamlead. (4) Verify chạy hàm GAS thật (sandbox stub) thay vì port tay → tránh drift.
- **Blocker:** **Dữ liệu User_Master còn thiếu** để đủ người nhận: (a) điền **Email** cho `MaiTTT7` + `QuynhNNY`; (b) thêm `TuanTT4@tpb.com.vn` vào `Cc_Extra` của `Report_Config`. Đều là chỉnh sheet [TT], **không cần redeploy** (đọc động).
- **Next step:** [TT] chỉnh 2 sheet trên → báo [CC] chạy lại `node send_email.js --dry` xác nhận đủ To + 6 Cc → `node run.js --send` gửi thử kỳ W34 → đổi scheduled task thứ 6 sang `run.js --send`.
- **Regression risk:** **Thấp.** `_reportConfig_` fallback về đúng hằng số cũ nếu sheet vắng (verify SR1–7 giữ nguyên 7/7 qua nhánh fallback); tổng **10/10** với SR8–10 (config override/Cc_Extra dedup/Enabled). AIOS chỉ thêm 1 dòng log ở `--dry`. Chưa gửi email thật nào.
